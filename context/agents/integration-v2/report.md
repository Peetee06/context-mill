...
call — the tool mirrors it into a shareable PostHog notebook and publishes it
to the wizard session. Do not write a report file, do not call
`notebooks-create`, and do not emit a `[NOTEBOOK_URL]` marker.
...
what the run could not confirm reads as unconfirmed. The tool's response names
the notebook URL (or says the notebook could not be created — that is not a
failure of your task; do not retry).
