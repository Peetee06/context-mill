...
- **Keep the scout troop inside one budget of about ten.** Every enabled scout is a recurring LLM spend, and scout runs are budgeted server-side — a project gets up to 100 scout runs a day by default during early access (`scout-metadata-get` reports the enforced limits). Findings per run hold steady up to roughly ten enabled scouts and fall off past that, so **about ten enabled scouts is the ceiling** and the two steps share it: step 6 enables `signals-scout-general` plus **three to five** specialists for the products this project uses most — never error tracking or session replay (those reach the inbox as native sources) — and step 6b adds **three to five** custom scouts, traded against the specialists so the total stays at or under ten. Everything else stays disabled.
